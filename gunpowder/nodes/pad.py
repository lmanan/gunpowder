        ...
        if self.mode=='constant':
            ...
            if a.shape==b.shape:
                ...
                if diff.ndim==3: # (C Y X)
                    ...
                    b[:, :diff[1], :diff[2]] = a[:, :diff[1], :diff[2]][:, ::-1, ::-1]
                elif diff.ndim==4: # (C Z Y X) 
                    b[:, :diff[1], diff[2]:, diff[3]:] = a[:, :diff[1], :, :][:, ::-1, :, :] # Z 
                    b[:, diff[1]:, :diff[2], diff[3]:] = a[:, :, :diff[2], :][:, :, ::-1, :] # Y
                    b[:, diff[1]:, diff[2]:, :diff[3]] = a[:, :, :, :diff[3]][:, :, :, ::-1] # X
                    b[:, :diff[1], :diff[2], :diff[3]] = a[:, :diff[1], :diff[2], :diff[3]][:, ::-1, ::-1, ::-1] 
